...
class Weather(LCD_Interface):
    ...
    To avoid waiting for the API request every time the script is run, recent
    weather data will be stored in a file. Each time the script runs, it can
    load the data from this file. Meanwhile, an API request can be initiated
    to update the file with the latest data.
    """
...
#        self.forecast_url = f"http://api.weatherapi.com/v1/forecast.json?key={self.api_key}&q={self.location}&days=2"
    ...
    def check_time_diff(self, filename):
        """
        Check if the difference between the current time and the time stored in
        a JSON file exceeds 15 minutes.

        Expects:
            - "time" key
            - stored in nested "current" dictionary

        Parameters:
            - filename (str or Path): Path to the JSON file.

        Returns:
            - bool: True if time difference exceeds 15 minutes, False otherwise.

        Raises:
            - ValueError: if the time is not stored as expected or missing 
        """
        # Load the json file into a dict
        data = self.load_data(filename)

        # Extract the time from the nested "current" dictionary
        current_data = data.get('current', {})
        json_time_str = current_data.get('time', None)

        if not json_time_str:
            raise ValueError(
                "The JSON file doesn't contain the expected time format.")
        
        # Parse the ISO8601 time (convert to datetime object)
        json_time = datetime.fromisoformat(json_time_str)

        # Get the current time
        current_time = datetime.now()

        # Calculate the difference
        time_difference = current_time - json_time

        # Check if the difference exceeds 15 minutes (returns Boolean T/F)
